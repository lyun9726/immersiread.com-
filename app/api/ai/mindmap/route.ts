/**
 * API Route: Generate Mindmap
 * POST /api/ai/mindmap
 */

import { NextRequest, NextResponse } from 'next/server';
import { generateMindmap } from '@/lib/ai/qwenService';
...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log(`[API] Generate mindmap for ${context.visibleText.length} chars`);

        const result = await generateMindmap(context, apiKey);
